...
function useRecorder() {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("hangeulai-warning", { detail: { message: String("Microphone access denied. Please allow microphone permissions in your browser and try again.") } }));
    }
  }, []);
...
export default function Phase5SpeakingLabWizard({ activeLesson, speakWord, onComplete,
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_phase5_step");
    const savedMax = localStorage.getItem("hangeulai_phase5_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_phase5_max_step", String(step));
    }
  }, [step, maxStep]);
...
return (
    ...
    setStep(2);
              ...
              <div className="pt-4 space-y-4">
                {!cChecked ? (
                  <button
                    ...
                    disabled={!cSelected}
                    className="w-full bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-zinc-955 font-black py-4 rounded-xl text-sm md:text-base transition duration-200 uppercase tracking-widest shadow-md shadow-brand-500/15 cursor-pointer"
                  >
                    Submit Answer
                  </button>
                ) : (
      ...
      {step === 12 && (
        <div className="glass-panel neon-border p-12 rounded-[2.5rem] shadow-2xl w-full space-y-8 flex-grow flex flex-col justify-center transition duration-300 animate-in fade-in duration-300">
          <div className="flex justify-between items-center border-b border-white/5 pb-4">
            <h2 className="text-2xl font-black text-white flex items-center gap-2 tracking-tight">
