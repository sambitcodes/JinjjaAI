...
export default function Course6Phase2IdiomsWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c6p2_step");
    const savedMax = localStorage.getItem("hangeulai_c6p2_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c6p2_max_step", String(step));
    }
  }, [step, maxStep]);
  ...
  const handleFinishPractice = async () => {
    ...
    { num: 2, label: "Screen 2 – C1 Idioms & Chunk categories" },
    { num: 3, label: "Screen 3 – Activity 1: Understand Idioms in Context (Dialogues, Collocations)" },
    { num: 4, label: "Screen 4 – Activity 2: Production (Gap-fill, Rewrites, AI chat)" },
    { num: 5, label: "Screen 5 – Mini-Quiz: Scenarios & Register checkpoints" },
    ...
    <div className="flex-grow flex flex-col justify-between">
      ...
      {step === 1 && (
        <div className="glass-panel neon-border p-12 rounded-[2.5rem] shadow-2xl w-full space-y-8 flex-grow flex flex-col justify-center text-center animate-fade-in">
          ...
          <h2 className="text-5xl font-black text-white tracking-tight font-sans">Korean 5.2</h2>
          <h3 className="text-2xl font-extrabold text-brand-400 mt-2">Idioms & Natural Expressions</h3>
