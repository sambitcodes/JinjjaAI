...
export default function Course4Phase4OpinionsWizard({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("hangeulai_c4p4_progress_state");
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.selectedStanceOpt !== undefined) setSelectedStanceOpt(state.selectedStanceOpt);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c4p4_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, c1Answer, c1Checked, c1Correct, c2Answer, c2Checked, c2Correct, c3Answer, c3Checked, c3Correct, recIdx, selectedStanceOpt, recChecked, recCorrect, selectedTopic, selectedStance, selectedReasons, quizIdx, quizChecked, quizCorrect, quizSelectedOpt, quizMistakes, quizScore, completedHomework]);
            ...
            <button
              ...
              onClick={() => {
                window.dispatchEvent(new CustomEvent("hangeulai-add-note", {
                  detail: {
                    question: `Course 4 Phase 4 Step ${step} - Study Concept`,
                    selected_answer: "Interactive Study Materials",
                    correct_answer: "Verified Korean Curriculum",
                    is_correct: true,
                    explanation: `Study notes for Course 4 Phase 4 Step ${step}.`
                  }
                }));
              }}
