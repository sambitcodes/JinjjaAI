...
export default function Phase1VowelBootcampWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_phase1_step");
    const savedMax = localStorage.getItem("hangeulai_phase1_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_phase1_max_step", String(step));
    }
  }, [step, maxStep]);
...
return (
  ...
  {step === outlineSteps.length && (p1QuizMistakes.length > 0  ) && (
    <div className="bg-zinc-900/60 p-6 rounded-2xl border border-red-500/20 text-left space-y-4 max-w-4xl mx-auto w-full mt-6 relative z-10">
      ...
      <div className="space-y-3">
        ...
        {(p1QuizMistakes || []).map((m: any, idx: number) => (
          <div key={idx} className="p-4 bg-zinc-955/80 rounded-xl border border-white/5 flex justify-between items-center text-left">
            ...
            <button
              onClick={() => {
                ...
                const bpIdx = (phase1Quiz || []).findIndex((q: any) => q.correct_answer === m || q.question === m || q.correctId === m);
                if (bpIdx !== -1) {
                  if (typeof setP1QuizIdx === "function") setP1QuizIdx(bpIdx);
                if (typeof setP1QuizChecked === "function") setP1QuizChecked(false);
                if (typeof setP1QuizCorrect === "function") setP1QuizCorrect(null);
                if (typeof setP1QuizScore === "function") setP1QuizScore(null);
                if (typeof setP1QuizSelected === "function") setP1QuizSelected(null);
                if (typeof setP1QuizWriting === "function") setP1QuizWriting("");
                }

                // Remove from mistakes list
                if (typeof setP1QuizMistakes === "function") setP1QuizMistakes((prev: any) => prev.filter((item: any) => item !== m));
              }}
