...
export default function Course4Phase5ParagraphsWizard({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("hangeulai_c4p5_progress_state");
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.selectedTopicOption !== undefined) setSelectedTopicOption(state.selectedTopicOption);
            if (state.selectedTopicSentence !== undefined) setSelectedTopicSentence(state.selectedTopicSentence);
            ...
            if (state.beginningText !== undefined) setBeginningText(state.beginningText);
            if (state.endText !== undefined) setEndText(state.endText);
            if (state.reflectionAnswer !== undefined) setReflectionAnswer(state.reflectionAnswer);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c4p5_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, c1Selected, c1Checked, c1Correct, c2Selected, c2Checked, c2Correct, readIdx, selectedTopicOption, selectedTopicSentence, activity1Checked, activity1Correct, selectedTopic, beginningText, endText, reflectionAnswer, quizIdx, quizChecked, quizCorrect, quizSelectedOpt, quizMistakes, quizScore, completedHomework]);
            ...
            <button
              ...
              onClick={() => {
                window.dispatchEvent(new CustomEvent("hangeulai-add-note", {
                  detail: {
                    question: `Course 4 Phase 5 Step ${step} - Study Concept`,
                    selected_answer: "Interactive Study Materials",
                    correct_answer: "Verified Korean Curriculum",
                    is_correct: true,
                    explanation: `Study notes for Course 4 Phase 5 Step ${step}.`
                  }
                }));
              }}
