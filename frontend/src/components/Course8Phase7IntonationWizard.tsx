...
export default function Course8Phase7IntonationWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c8p7_step");
    const savedMax = localStorage.getItem("hangeulai_c8p7_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c8p7_max_step", String(step));
    }
  }, [step, maxStep]);
      ...
      try {
        ...
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.sYnCorrect !== undefined) setSYnCorrect(state.sYnCorrect);
            if (state.ynWhIdx !== undefined) setYnWhIdx(state.ynWhIdx);
            if (state.ynWhSelectedYn !== undefined) setYnWhSelectedYn(state.ynWhSelectedYn);
            if (state.ynWhSelectedWh !== undefined) setYnWhSelectedWh(state.ynWhSelectedWh);
            if (state.ynWhChecked !== undefined) setYnWhChecked(state.ynWhChecked);
            if (state.ynWhCorrect !== undefined) setYnWhCorrect(state.ynWhCorrect);
            if (state.elIdx !== undefined) setElIdx(state.elIdx);
            if (state.elSelected !== undefined) setElSelected(state.elSelected);
            if (state.elChecked !== undefined) setElChecked(state.elChecked);
            if (state.elCorrect !== undefined) setElCorrect(state.elCorrect);
            if (state.sqIdx !== undefined) setSqIdx(state.sqIdx);
            if (state.sqScore !== undefined) setSqScore(state.sqScore);
            if (state.wqIdx !== undefined) setWqIdx(state.wqIdx);
            if (state.wqScore !== undefined) setWqScore(state.wqScore);
            if (state.epIdx !== undefined) setEpIdx(state.epIdx);
            if (state.epScore !== undefined) setEpScore(state.epScore);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c8p7_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, sYnIdx, sYnSelected, sYnChecked, sYnCorrect, ynWhIdx, ynWhSelectedYn, ynWhSelectedWh, ynWhChecked, ynWhCorrect, elIdx, elSelected, elChecked, elCorrect, sqIdx, sqScore, wqIdx, wqScore, epIdx, epScore, quizIdx, quizSelected, quizChecked, quizCorrect, quizMistakes, quizScore]);
  ...
  const handleCompleteLab = async () => {
    ...
    { num: 2, label: "= 2 ? \"bg-teal-500\" : \"bg-slate-700\"}`} /> Screen 2: Pitch Contours & Emotion Ranges" },
    { num: 3, label: "= 3 ? \"bg-indigo-500\" : \"bg-slate-700\"}`} /> Screen 3: Activity 1: Listen & Discriminate" },
    { num: 4, label: "= 4 ? \"bg-purple-500\" : \"bg-slate-700\"}`} /> Screen 4: Activity 2: Speaking Evaluation" },
    { num: 5, label: "= 5 ? \"bg-pink-500\" : \"bg-slate-700\"}`} /> Screen 5: Mini-Quiz: Intonation Mastery" },
    { num: 6, label: "= 6 ? \"bg-emerald-500\" : \"bg-slate-700\"}`} /> Screen 6: Homework & Final Completion" }
  ];
    ...
    <div className="flex-grow flex flex-col justify-between">
      ...
      <header className="border-b border-white/5 bg-zinc-900/60 backdrop-blur px-8 py-5 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-black text-xl text-white tracking-tight">Intonation Lab</h2>
        ...
        return (
          <div className="mb-6 p-5 bg-zinc-955/80 rounded-3xl border border-white/5 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300 relative z-30">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-[10px] font-black text-zinc-400 bg-zinc-900 border border-white/10 px-2 py-0.5 rounded">Required: 660 XP</span>
            </div>
                    ...
                    onClick={() => {
                      if (courseXP < 480) {
                        window.dispatchEvent(new CustomEvent("hangeulai-warning", {
                          detail: { message: "To start Phase 7, you need at least 480 XP in this course. You currently have " + courseXP + " XP." }
                        }));
                        return;
                      }
                      setStep(s.num);
      ...
      {step === 1 && (
        <div className="glass-panel neon-border p-12 rounded-[2.5rem] shadow-2xl w-full space-y-8 flex-grow flex flex-col justify-center text-center animate-fade-in relative overflow-hidden">
          ...
          <div>
            <h2 className="text-5xl font-black text-white tracking-tight font-sans">{metadata?.title || "Intonation Lab – Questions, Statements & Feelings (B1)"}</h2>
            <h3 className="text-2xl font-extrabold text-cyan-400 mt-2">{metadata?.subtitle || "Use pitch to show meaning and emotion."}</h3>
          </div>
          ...
          <div className="flex flex-col gap-3 max-w-xs mx-auto pt-2">
            <button 
              onClick={() => {
    if (courseXP < 480) {
      window.dispatchEvent(new CustomEvent("hangeulai-warning", { detail: { message: String("To start Phase 7, you need at least 480 XP in this course. You currently have " + courseXP + " XP. Please complete earlier steps/phases to earn more XP!") } }));
      return;
    }
    setStep(2);
  }} 
