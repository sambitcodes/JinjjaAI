...
export default function Course3Phase4PlansWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c3p4_step");
    const savedMax = localStorage.getItem("hangeulai_c3p4_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c3p4_max_step", String(step));
    }
  }, [step, maxStep]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("hangeulai_c3p4_progress_state");
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.conjugationSelected !== undefined) setConjugationSelected(state.conjugationSelected);
            if (state.conjugationChecked !== undefined) setConjugationChecked(state.conjugationChecked);
            if (state.conjugationCorrect !== undefined) setConjugationCorrect(state.conjugationCorrect);
            if (state.timeIdx !== undefined) setTimeIdx(state.timeIdx);
            if (state.timeSelected !== undefined) setTimeSelected(state.timeSelected);
            if (state.timeChecked !== undefined) setTimeChecked(state.timeChecked);
            if (state.timeCorrect !== undefined) setTimeCorrect(state.timeCorrect);
            ...
            if (state.selectedHorizonId !== undefined) setSelectedHorizonId(state.selectedHorizonId);
            if (state.selectedActivities !== undefined) setSelectedActivities(state.selectedActivities);
            if (state.selectedReasons !== undefined) setSelectedReasons(state.selectedReasons);
            if (state.builderReflectionSelected !== undefined) setBuilderReflectionSelected(state.builderReflectionSelected);
            if (state.builderReflectionChecked !== undefined) setBuilderReflectionChecked(state.builderReflectionChecked);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c3p4_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, cSelected, cChecked, cCorrect, cIdx, answeredConcepts, flippedTimeIdx, conjugationIdx, conjugationSelected, conjugationChecked, conjugationCorrect, timeIdx, timeSelected, timeChecked, timeCorrect, listeningIdx, selectedSummaryId, listeningChecked, listeningCorrect, detailAnswers, detailChecked, detailCorrect, selectedHorizonId, selectedActivities, selectedReasons, builderReflectionSelected, builderReflectionChecked, quizIdx, quizChecked, quizCorrect, quizSelectedOpt, quizWritingAns, quizScore, quizMistakes, completedHomework]);
  ...
  const handleBuildParagraph = async () => {
    if (selectedActivities.length === 0) {
      window.dispatchEvent(new CustomEvent("hangeulai-warning", { detail: { message: String("Please select at least one activity plan.") } }));
      return;
    }
    setBuilding(true);
