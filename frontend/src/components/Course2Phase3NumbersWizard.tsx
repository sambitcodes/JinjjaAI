...
export default function Course2Phase3NumbersWizard({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("hangeulai_c2p3_progress_state");
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.selectedTimeOpt !== undefined) setSelectedTimeOpt(state.selectedTimeOpt);
            ...
            if (state.factsIdx !== undefined) setFactsIdx(state.factsIdx);
            if (state.selectedFactOpt !== undefined) setSelectedFactOpt(state.selectedFactOpt);
            if (state.factsChecked !== undefined) setFactsChecked(state.factsChecked);
            if (state.factsCorrect !== undefined) setFactsCorrect(state.factsCorrect);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c2p3_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, cSelected, cChecked, cCorrect, cIdx, answeredConcepts, activeCatalogIdx, catalogSelectedOpt, catalogChecked, catalogCorrect, listeningIdx, selectedListeningOpt, listeningChecked, listeningCorrect, timeIdx, selectedTimeOpt, timeChecked, timeCorrect, factsIdx, selectedFactOpt, factsChecked, factsCorrect, quizIdx, quizChecked, quizCorrect, quizSelectedOpt, quizWritingAns, quizScore, quizMistakes, completedHomework]);
