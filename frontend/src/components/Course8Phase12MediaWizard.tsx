...
export default function Course8Phase12MediaWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c8p12_step");
    const savedMax = localStorage.getItem("hangeulai_c8p12_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c8p12_max_step", String(step));
    }
  }, [step, maxStep]);
      ...
      try {
        ...
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.opinionIdx !== undefined) setOpinionIdx(state.opinionIdx);
            if (state.opinionSelected !== undefined) setOpinionSelected(state.opinionSelected);
            if (state.opinionChecked !== undefined) setOpinionChecked(state.opinionChecked);
            if (state.opinionCorrect !== undefined) setOpinionCorrect(state.opinionCorrect);
            if (state.reasonsIdx !== undefined) setReasonsIdx(state.reasonsIdx);
            if (state.selectedReasons !== undefined) setSelectedReasons(state.selectedReasons);
            if (state.reasonsChecked !== undefined) setReasonsChecked(state.reasonsChecked);
            if (state.summaryIdx !== undefined) setSummaryIdx(state.summaryIdx);
            if (state.recordingProgress !== undefined) setRecordingProgress(state.recordingProgress);
            if (state.opinionReactIdx !== undefined) setOpinionReactIdx(state.opinionReactIdx);
            if (state.retellIdx !== undefined) setRetellIdx(state.retellIdx);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c8p12_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, gistIdx, gistSelected, gistChecked, gistCorrect, opinionIdx, opinionSelected, opinionChecked, opinionCorrect, reasonsIdx, selectedReasons, reasonsChecked, summaryIdx, recordingProgress, opinionReactIdx, retellIdx, quizIdx, quizSelected, quizChecked, quizCorrect, quizMistakes, quizScore]);
        ...
        return (
          <div className="mb-6 p-5 bg-zinc-955/80 rounded-3xl border border-white/5 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300 relative z-30">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-[10px] font-black text-zinc-400 bg-zinc-900 border border-white/10 px-2 py-0.5 rounded">Required: 1060 XP</span>
            </div>
                    ...
                    onClick={() => {
                      if (courseXP < 880) {
                        window.dispatchEvent(new CustomEvent("hangeulai-warning", {
                          detail: { message: "To start Phase 12, you need at least 880 XP in this course. You currently have " + courseXP + " XP." }
                        }));
                        return;
                      }
                      setStep(s.num);
        ...
        <div className="glass-panel border border-yellow-500/20 p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center text-center relative overflow-hidden">
          ...
          <div className="flex flex-col gap-3 max-w-xs mx-auto pt-2">
            <button 
              onClick={() => {
    if (courseXP < 880) {
      window.dispatchEvent(new CustomEvent("hangeulai-warning", { detail: { message: String("To start Phase 12, you need at least 880 XP in this course. You currently have " + courseXP + " XP. Please complete earlier steps/phases to earn more XP!") } }));
      return;
    }
    setStep(2);
  }} 
