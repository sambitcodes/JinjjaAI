...
// --- Start Progress State Preservation ---
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("hangeulai_c5p5_progress_state");
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.listIdx !== undefined) setListIdx(state.listIdx);
            if (state.selectedMainIdea !== undefined) setSelectedMainIdea(state.selectedMainIdea);
            if (state.selectedDetails !== undefined) setSelectedDetails(state.selectedDetails);
            ...
            if (state.selectedTemplateType !== undefined) setSelectedTemplateType(state.selectedTemplateType);
            if (state.learnerSummaryText !== undefined) setLearnerSummaryText(state.learnerSummaryText);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c5p5_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, c1Selected, c1Checked, listIdx, selectedMainIdea, selectedDetails, act1Checked, act1Correct, selectedTemplateType, learnerSummaryText, quizIdx, quizChecked, quizCorrect, quizSelectedOpt, quizMistakes, quizScore, practiceText, practiceFinished, completedHomework]);
            ...
            <button
              ...
              onClick={() => {
                window.dispatchEvent(new CustomEvent("hangeulai-add-note", {
                  detail: {
                    question: `Course 5 Phase 5 Step ${step} - Study Concept`,
                    selected_answer: "Interactive Study Materials",
                    correct_answer: "Verified Korean Curriculum",
                    is_correct: true,
                    explanation: `Study notes for Course 5 Phase 5 Step ${step}.`
                  }
                }));
              }}
