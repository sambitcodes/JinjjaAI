...
export default function Course8Phase10ReactionWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c8p10_step");
    const savedMax = localStorage.getItem("hangeulai_c8p10_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c8p10_max_step", String(step));
    }
  }, [step, maxStep]);
      ...
      try {
        ...
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.ynChecked !== undefined) setYnChecked(state.ynChecked);
            if (state.ynCorrect !== undefined) setYnCorrect(state.ynCorrect);
            if (state.brIdx !== undefined) setBrIdx(state.brIdx);
            if (state.brSelected !== undefined) setBrSelected(state.brSelected);
            if (state.brChecked !== undefined) setBrChecked(state.brChecked);
            if (state.brCorrect !== undefined) setBrCorrect(state.brCorrect);
            if (state.bingoIdx !== undefined) setBingoIdx(state.bingoIdx);
            if (state.bingoPromptIdx !== undefined) setBingoPromptIdx(state.bingoPromptIdx);
            if (state.bingoChecked !== undefined) setBingoChecked(state.bingoChecked);
            if (state.bingoScore !== undefined) setBingoScore(state.bingoScore);
            if (state.faIdx !== undefined) setFaIdx(state.faIdx);
            if (state.faPrepCountdown !== undefined) setFaPrepCountdown(state.faPrepCountdown);
            if (state.faScore !== undefined) setFaScore(state.faScore);
            if (state.speedRoundFinished !== undefined) setSpeedRoundFinished(state.speedRoundFinished);
            if (state.clIdx !== undefined) setClIdx(state.clIdx);
            if (state.clScore !== undefined) setClScore(state.clScore);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c8p10_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, ynIdx, ynCountdown, ynSelected, ynChecked, ynCorrect, brIdx, brSelected, brChecked, brCorrect, bingoIdx, bingoPromptIdx, bingoChecked, bingoScore, faIdx, faPrepCountdown, faScore, speedRoundFinished, clIdx, clScore, quizIdx, quizSelected, quizChecked, quizCorrect, quizMistakes, quizScore]);
  ...
  const handleCompleteLab = async () => {
    ...
    { num: 2, label: "= 2 ? \"bg-teal-500\" : \"bg-slate-700\"}`} /> Screen 2: Immediate Response Skills" },
    { num: 3, label: "= 3 ? \"bg-indigo-500\" : \"bg-slate-700\"}`} /> Screen 3: Activity 1: Click Reactions & Bingo" },
    { num: 4, label: "= 4 ? \"bg-purple-500\" : \"bg-slate-700\"}`} /> Screen 4: Activity 2: Fast Spoken Drills" },
    { num: 5, label: "= 5 ? \"bg-pink-500\" : \"bg-slate-700\"}`} /> Screen 5: Mini-Quiz: Speed & Pragmatics" },
    { num: 6, label: "= 6 ? \"bg-emerald-500\" : \"bg-slate-700\"}`} /> Screen 6: Homework & Response Latency Coach" }
  ];
    ...
    <div className="flex-grow flex flex-col justify-between">
      ...
      <header className="border-b border-white/5 bg-zinc-900/60 backdrop-blur px-8 py-5 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-black text-xl text-white tracking-tight">Reaction Lab</h2>
        ...
        return (
          <div className="mb-6 p-5 bg-zinc-955/80 rounded-3xl border border-white/5 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300 relative z-30">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-[10px] font-black text-zinc-400 bg-zinc-900 border border-white/10 px-2 py-0.5 rounded">Required: 900 XP</span>
            </div>
                    ...
                    onClick={() => {
                      if (courseXP < 720) {
                        window.dispatchEvent(new CustomEvent("hangeulai-warning", {
                          detail: { message: "To start Phase 10, you need at least 720 XP in this course. You currently have " + courseXP + " XP." }
                        }));
                        return;
                      }
                      setStep(s.num);
      ...
      {step === 1 && (
        <div className="glass-panel neon-border p-12 rounded-[2.5rem] shadow-2xl w-full space-y-8 flex-grow flex flex-col justify-center text-center animate-fade-in relative overflow-hidden">
          ...
          <div>
            <h2 className="text-5xl font-black text-white tracking-tight font-sans">{metadata?.title || "Reaction Lab – Listen & Respond Fast (B1)"}</h2>
            <h3 className="text-2xl font-extrabold text-cyan-400 mt-2">{metadata?.subtitle || "Answer quickly and naturally."}</h3>
          </div>
          ...
          <div className="flex flex-col gap-3 max-w-xs mx-auto pt-2">
            <button 
              onClick={() => {
    if (courseXP < 720) {
      window.dispatchEvent(new CustomEvent("hangeulai-warning", { detail: { message: String("To start Phase 10, you need at least 720 XP in this course. You currently have " + courseXP + " XP. Please complete earlier steps/phases to earn more XP!") } }));
      return;
    }
    setStep(2);
  }} 
