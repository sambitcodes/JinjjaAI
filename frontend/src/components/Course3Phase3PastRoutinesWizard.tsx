...
export default function Course3Phase3PastRoutinesWizard({
  ...
  const [flippedWordIdx, setFlippedWordIdx] = useState<number | null>(null);
  ...
  const [builderSlots, setBuilderSlots] = useState<any[]>([
    { slot_name: "Morning", verb_index: 0, past_ko: "" },
    { slot_name: "Daytime", verb_index: 0, past_ko: "" },
    { slot_name: "Evening", verb_index: 0, past_ko: "" }
  ]);
  ...
  // Transform Drill check
  const handleCheckDrill = async () => {
    const current = transformDrills[drillIdx];
    if (!current || !drillSelectedOpt) return;

    const isCorrect = drillSelectedOpt === current.correct_answer;
  ...
  // Activity 2 Builder slot dropdown builder
  ...
  const handleSpeechEvaluate = async () => {
    const target = builtParagraph ? builtParagraph.final_korean_text : "어제 아침에 일찍 일어났어요.";
    ...
    <div className="flex-grow flex flex-col justify-between max-w-2xl mx-auto w-full">
      ...
      <header className="flex justify-between items-center py-4 border-b border-white/5 mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 font-medium">Curated Topic: Korean Simple Past Tense Conjugation</p>
          </div>
        </div>
      ...
      {step === 1 && (
        <div className="glass-panel neon-border p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center text-center">
          ...
          <h2 className="text-4xl font-black text-white">Korean 2.3</h2>
          <h3 className="text-xl font-bold text-brand-400 mt-1">Talking About Yesterday</h3>
          ...
          {showOutline && (
            <div className="bg-zinc-950 p-4 rounded-xl border border-white/5 text-left text-xs text-zinc-400 space-y-1.5 animate-fade-in max-w-md mx-auto w-full font-mono">
              ...
              <p>✓ Activity 1 – Past tense verb grids & card flips</p>
              <p>✓ Activity 2 – Past time expressions</p>
              <p>✓ Activity 3 – Yesterday routine listening summaries</p>
              <p>✓ Activity 4 – Present-to-past transform drills</p>
              <p>✓ Activity 5 – Yesterday timeline builder & speaking practice</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="glass-panel neon-border p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center">
          <div className="flex justify-between items-center border-b border-white/5 pb-4">
            <h2 className="text-xl font-black text-white flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-brand-400" />
              <span>Simple Past for Daily Life</span>
            </h2>
          ...
          <div className="bg-zinc-900/60 p-4 rounded-xl border border-white/5 space-y-2 text-xs leading-relaxed text-zinc-300 text-left">
            <p><strong>Conjugating Daily Verbs:</strong> Add <strong>~았어요/었어요</strong> based on the vowel rule. Verbs ending in <strong>하다</strong> change to <strong>했어요</strong>.</p>
          </div>

          {/* Past Verbs Grid */}
          <div className="space-y-2 text-left">
            <span className="text-[10px] text-zinc-500 font-black uppercase tracking-wider block">1. Verb Past Conjugations</span>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 max-h-[30vh] overflow-y-auto pr-1">
              {coreData?.past_verbs?.map((v: any, idx: number) => {
                const isFlipped = flippedWordIdx === idx;
                return (
                  <div
                    key={idx}
                    onClick={() => {
                      setFlippedWordIdx(isFlipped ? null : idx);
                      playAudio(v.past);
                    }}
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-[8px] font-mono text-zinc-500 uppercase tracking-widest">{v.dictionary}</span>
                      <button onClick={(e) => { e.stopPropagation(); playAudio(v.past); }} className="p-1 rounded bg-zinc-950/40 text-zinc-400"><Volume2 className="w-3.5 h-3.5" /></button>
                    </div>

                    {!isFlipped ? (
                      <div className="py-1">
                        <div className="text-base font-black text-white font-korean">{v.past}</div>
                        <div className="text-[9px] text-zinc-500">Present: {v.present}</div>
                      </div>
                    ) : (
                      <div className="animate-fade-in text-left space-y-0.5">
                        <div className="text-xs font-black text-white capitalize">{v.english}</div>
                        <p className="text-[8px] text-zinc-500 font-mono leading-none">{v.romanization}</p>
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="space-y-2 text-left">
            <span className="text-[10px] text-zinc-500 font-black uppercase tracking-wider block">2. Past Time Expressions</span>
            <div className="flex flex-wrap gap-2 max-h-[15vh] overflow-y-auto pr-1">
              {coreData?.past_time_expressions?.map((t: any, idx: number) => (
                <div key={idx} onClick={() => playAudio(t.ko)} className="p-3 bg-zinc-950 hover:bg-zinc-900 cursor-pointer rounded-xl border border-white/5 flex-grow text-xs text-left min-w-[120px] transition">
          ...
          {/* Example Past Routine */}
          {coreData?.example_past_routines?.[0] && (
            <div className="space-y-2 text-left">
              <span className="text-[10px] text-zinc-500 font-black uppercase tracking-wider block">3. Example Past Routine Story</span>
              <div className="p-4 bg-zinc-950 rounded-xl border border-white/5 space-y-3">
                <p className="font-korean font-bold text-sm leading-relaxed text-white">
                  <span className="text-amber-300">어제</span> 저는 아침 일찍 <span className="text-cyan-300">일어났어요</span>. 그리고 학교에 <span className="text-cyan-300">갔어요</span>. 오후에 친구를 <span className="text-cyan-300">만났어요</span>. 우리는 영화를 <span className="text-cyan-300">봤어요</span>. 밤에 일찍 <span className="text-cyan-300">잤어요</span>.
                </p>
                <p className="text-xs text-zinc-400">{coreData.example_past_routines[0].en}</p>
                <div className="flex justify-between items-center">
                  <span className="text-[9px] text-zinc-500 font-mono italic">Note: Yellow = past time, Blue = past verb</span>
                  <button onClick={() => playAudio(coreData.example_past_routines[0].ko)} className="p-2 rounded bg-zinc-900 text-zinc-400 hover:text-white"><Volume2 className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="glass-panel neon-border p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center">
          <div className="flex justify-between items-center border-b border-white/5 pb-4">
            <h2 className="text-xl font-black text-white flex items-center gap-2">
              ...
              <span>Activity 1 – Yesterday's Routine Listening</span>
            </h2>
            ...
            <div className="space-y-6 max-w-xl mx-auto w-full">
              ...
              <div className="bg-zinc-900/30 p-5 rounded-2xl border border-white/5 space-y-4">
                <div className="text-left text-center">
                  <span className="text-[9px] text-zinc-500 font-black uppercase tracking-wider block">Part A: Listen & Choose Correct English Summary</span>
                  <p className="text-xs text-zinc-300 mt-1">Listen to what they did yesterday:</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-2 max-w-md mx-auto">
                  {listeningItems[listeningIdx]?.options.map((opt: any) => (
                    <button
                      ...
                      } ${listeningChecked && opt.correct ? "border-accent-teal bg-accent-teal/15 text-white" : ""}`}
                    >
                      {opt.text}
                    </button>
                  ))}
                ...
                {listeningChecked && (
                  <div className={`p-3 rounded-xl border text-xs text-center ${
                    ...
                    {listeningCorrect ? "Correct! Well comprehended." : "Incorrect summary option selected."}
                  </div>
                )}
      ...
      {/* Screen 4: Activity 2: Present to Past Transformation & Routine Builder */}
      {step === 4 && (
        <div className="glass-panel neon-border p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center">
          <div className="flex justify-between items-center border-b border-white/5 pb-4">
            <h2 className="text-xl font-black text-white flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-brand-400" />
              <span>Activity 2 – Present to Past & Builder</span>
            </h2>
          ...
          {transformDrills.length > 0 && (
            <div className="bg-zinc-900/30 p-5 rounded-2xl border border-white/5 space-y-4">
              <h3 className="text-xs font-black uppercase text-amber-400 tracking-wider text-left">Part A: Present → Past Transformation</h3>
              <div className="space-y-3">
                <p className="text-xs text-zinc-300 text-left">
                  Transform this present-tense sentence into the past: <strong>"{transformDrills[drillIdx]?.present}"</strong>
                </p>
                    ...
                    <button
                      ...
                      className={`p-2.5 rounded-xl border text-left text-xs font-bold transition ${
                        drillSelectedOpt === opt
                          ? "border-brand-500 bg-brand-500/10 text-white"
                          : "border-white/5 bg-zinc-950 text-zinc-400 hover:border-white/10"
                      } ${drillChecked && opt === transformDrills[drillIdx]?.correct_answer ? "border-accent-teal bg-accent-teal/15 text-white" : ""}`}
                    >
                      {opt}
                    </button>
                  ))}
                ...
                {drillChecked && (
                  <div className={`p-3 rounded-xl border text-xs text-left ${
                    ...
                    <p className="font-bold">{drillCorrect ? "Correct!" : "Incorrect."}</p>
                    <p className="mt-1">{transformDrills[drillIdx]?.explanation}</p>
                  </div>
                )}
          ...
          {/* Builder Part */}
          <div className="bg-zinc-900/30 p-5 rounded-2xl border border-white/5 space-y-4">
            <h3 className="text-xs font-black uppercase text-amber-400 tracking-wider text-left">Part B: "Yesterday/Last Weekend" Builder</h3>
            ...
            <div className="space-y-3">
              {builderSlots.map((slot, idx) => {
                ...
                return (
                  <div key={idx} className="p-3 bg-zinc-950 rounded-xl border border-white/5 text-left space-y-2">
                    <span className="text-xs font-bold text-white">{slot.slot_name} Activity:</span>
                    <select
                      value={slot.past_ko}
            ...
            <div className="flex justify-center pt-1">
              ...
              >
                {building ? "Assembling..." : "Assemble Story"}
              </button>
            </div>

            {builtParagraph && (
              <div className="bg-zinc-950 p-4 rounded-xl border border-brand-500/25 space-y-3 animate-fade-in text-center">
                <span className="text-[9px] text-brand-400 font-black uppercase tracking-wider block">Assembled Past Story</span>
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  >
                    {savingParagraph ? "Saving..." : paragraphSaved ? "Saved!" : "Save to Profile"}
                  </button>
                </div>

                {/* Hold to record voice evaluation */}
                <div className="pt-4 border-t border-white/5 space-y-3">
                  <span className="text-[9px] text-zinc-500 font-mono uppercase tracking-wider block">Part C: Record & Verify Speaking</span>
                  ...
                  {speakingTranscribing && (
                    <div className="flex items-center justify-center gap-2 text-xs text-zinc-500">
                      ...
                      <span>Gwan-Sik transcribing your voice...</span>
                    </div>
                  )}
                  ...
                  {speakingResult && (
                    <div className="p-3 bg-zinc-900 rounded-xl border border-white/5 text-left text-xs space-y-1">
                      <div className="flex justify-between items-center">
                        <span className="text-zinc-400 font-bold">Acoustic Score:</span>
                        <span className="text-accent-teal font-black">{speakingResult.similarity_score}%</span>
                      </div>
                      <p className="text-zinc-500 text-[10px]">Recognized text: "{speakingResult.recognized_text}"</p>
        ...
        <div className="glass-panel neon-border p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center">
          <div className="flex justify-between items-center border-b border-white/5 pb-4">
            <h2 className="text-xl font-black text-white flex items-center gap-2">
              ...
              <span>Step 5 – Mini-Quiz (Phase 3 Checkpoint)</span>
            </h2>
      ...
      {step === 6 && (
        <div className="glass-panel neon-border p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center text-center max-w-xl mx-auto">
          ...
          <div>
            <h2 className="text-2xl font-black text-white">Phase 3 Complete! 🇰🇷✨</h2>
            <p className="text-zinc-400 text-xs mt-1">You scored {quizScore}% on the past routines checkpoint.</p>
          </div>
          ...
          <div className="bg-zinc-950 p-4 rounded-xl border border-brand-500/10 space-y-3">
            <div className="text-left space-y-1 text-center">
              <span className="text-[9px] text-brand-400 font-black uppercase tracking-wider block">Start Speaking Practice with Gwan-Sik</span>
              <p className="text-[11px] text-zinc-500">Practice describing your yesterday routine in detail with live feedback.</p>
            </div>
