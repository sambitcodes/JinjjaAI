...
export default function Course4Phase6CapstoneWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c4p6_step");
    const savedMax = localStorage.getItem("hangeulai_c4p6_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c4p6_max_step", String(step));
    }
  }, [step, maxStep]);
...
// --- Start Progress State Preservation ---
  ...
  const renderIcon = (iconName: string) => {
    ...
    { num: 2, label: "Activity 1 – Guided Conversational Analysis & missing lines" },
    { num: 3, label: "Activity 2 – Live AI Conversation Tutor (Text/Voice) Simulation" },
    { num: 4, label: "Activity 3 – Conversational coherence checkpoint quiz" },
        ...
        <div className="glass-panel neon-border p-8 rounded-3xl shadow-2xl w-full space-y-6 flex-grow flex flex-col justify-center text-center max-w-xl mx-auto">
          ...
          <div className="p-4 bg-zinc-950 rounded-2xl border border-white/5 space-y-3">
            ...
            <div className="flex gap-2">
              <button 
                onClick={() => {
                  window.dispatchEvent(new CustomEvent("hangeulai-warning", { detail: { message: String("Initiating Full Capstone: Hobbies & Opinions") } }));
              ...
              <button 
                onClick={() => {
                  window.dispatchEvent(new CustomEvent("hangeulai-warning", { detail: { message: String("Initiating Full Capstone: Daily Life & Story") } }));
