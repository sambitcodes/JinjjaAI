...
// --- Start Progress State Preservation ---
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      try {
        const saved = localStorage.getItem("hangeulai_c5p4_progress_state");
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.selectedListener !== undefined) setSelectedListener(state.selectedListener);
            if (state.selectedApprop !== undefined) setSelectedApprop(state.selectedApprop);
            ...
            if (state.activeTransIdx !== undefined) setActiveTransIdx(state.activeTransIdx);
            if (state.selectedContext !== undefined) setSelectedContext(state.selectedContext);
            if (state.selectedTransformOption !== undefined) setSelectedTransformOption(state.selectedTransformOption);
            if (state.selectedSoftener !== undefined) setSelectedSoftener(state.selectedSoftener);
            if (state.selectedBaseIdea !== undefined) setSelectedBaseIdea(state.selectedBaseIdea);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c5p4_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, c1Selected, c1Checked, c1Correct, recIdx, selectedListener, selectedApprop, act1Checked, act1Correct, activeTransIdx, selectedContext, selectedTransformOption, selectedSoftener, selectedBaseIdea, roleplayText, roleplayFinished, quizIdx, quizChecked, quizCorrect, quizSelectedOpt, quizMistakes, quizScore, practiceText, practiceFinished, completedHomework]);
            ...
            <button
              ...
              onClick={() => {
                window.dispatchEvent(new CustomEvent("hangeulai-add-note", {
                  detail: {
                    question: `Course 5 Phase 4 Step ${step} - Study Concept`,
                    selected_answer: "Interactive Study Materials",
                    correct_answer: "Verified Korean Curriculum",
                    is_correct: true,
                    explanation: `Study notes for Course 5 Phase 4 Step ${step}.`
                  }
                }));
              }}
