...
export default function Course8Phase6ConnectedSpeechWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c8p6_step");
    const savedMax = localStorage.getItem("hangeulai_c8p6_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c8p6_max_step", String(step));
    }
  }, [step, maxStep]);
