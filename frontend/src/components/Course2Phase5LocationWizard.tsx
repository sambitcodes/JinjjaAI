...
export default function Course2Phase5LocationWizard({
  ...
  useEffect(() => {
    const savedStep = localStorage.getItem("hangeulai_c2p5_step");
    const savedMax = localStorage.getItem("hangeulai_c2p5_max_step");
    let currentParsed = 1;
    if (savedStep) {
  ...
  useEffect(() => {
    if (step > maxStep) {
      setMaxStep(step);
      localStorage.setItem("hangeulai_c2p5_max_step", String(step));
    }
  }, [step, maxStep]);
