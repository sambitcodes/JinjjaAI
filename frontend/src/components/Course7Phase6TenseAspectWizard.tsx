      ...
      try {
        ...
        if (saved) {
          const state = JSON.parse(saved);
            ...
            if (state.hvsIdx !== undefined) setHvsIdx(state.hvsIdx);
            if (state.hvsInput !== undefined) setHvsInput(state.hvsInput);
            if (state.hvsChecked !== undefined) setHvsChecked(state.hvsChecked);
            if (state.hvsCorrect !== undefined) setHvsCorrect(state.hvsCorrect);
            if (state.bgeIdx !== undefined) setBgeIdx(state.bgeIdx);
            if (state.bgeInput !== undefined) setBgeInput(state.bgeInput);
            if (state.bgeChecked !== undefined) setBgeChecked(state.bgeChecked);
            if (state.bgeCorrect !== undefined) setBgeCorrect(state.bgeCorrect);
            if (state.tsIdx !== undefined) setTsIdx(state.tsIdx);
            if (state.tsInput !== undefined) setTsInput(state.tsInput);
            if (state.tsChecked !== undefined) setTsChecked(state.tsChecked);
            if (state.tsCorrect !== undefined) setTsCorrect(state.tsCorrect);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        localStorage.setItem("hangeulai_c7p6_progress_state", JSON.stringify(state));
      } catch (e) {
        console.error("Failed to save progress state:", e);
      }
    }
  }, [step, maxStep, recIdx, recSelected, recChecked, recCorrect, hvsIdx, hvsInput, hvsChecked, hvsCorrect, bgeIdx, bgeInput, bgeChecked, bgeCorrect, tsIdx, tsInput, tsChecked, tsCorrect, quizIdx, quizSelected, quizChecked, quizCorrect, quizMistakes, quizScore]);
